...
function FitToViewport({ baseWidth = 380, children }: { baseWidth?: number; children: ReactNode }) {
  ...
  return (
    <div ref={containerRef} className="min-w-0 min-h-0 w-full h-full overflow-hidden flex justify-center">
      <div
        style={{
          width: baseWidth,
      ...
      >
        <div ref={innerRef} style={{ width: baseWidth, height: "100%" }}>
          {children}
        </div>
      </div>
    </div>
...
export function GamesLauncher({ open, onClose, initialGame = null, compact = false }: Props) {
  ...
  const shellSize = compact
    ...
    : "w-full h-full max-w-full h-[100dvh] max-h-[100dvh] rounded-none border-none";
  ...
  return (
    <div className={
      ...
      <div className={`relative w-full min-h-0 ${shellSize} overflow-hidden bg-neutral-950 shadow-2xl flex flex-col`} onClick={(e) => e.stopPropagation()}>
        ...
        {/* Body */}
        <div className={`min-h-0 flex-1 ${activeGame ? "overflow-hidden relative" : "overflow-y-auto"}`}>
          {error && (
